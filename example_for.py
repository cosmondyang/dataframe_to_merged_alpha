...
* Load a large feature parquet (
generated by ``gen_all_date_feature.py``).
...
class TouchstoneMetricBook:
    ...
    def evaluate(self, series: pd.Series, label: str) -> MetricResult:
        ...
        print(factor_df)
        try:
            new = factor_df.groupby(level=factor_df.index.names[0])[label] \
            .transform(lambda x: (x - x.mean()) / x.std())
            res = analyze(new, state=self.state, T=self.T)
