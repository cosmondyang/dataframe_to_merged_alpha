...
``MetricBook``
    Computes IC and ``lw_ic_v2`` (the latter is a daily IC weighted by the
    square root of the cross-sectional sample size – a simple proxy for the
    liquidity-weighted IC mentioned in the original document).
...
class AlphaWorkspace:
    ...
    def _apply_preprocess(self, col: str, strategy: str) -> pd.Series:
        ...
        if strategy == "fill_cs_mean":
            ...
            filled = (
                data.groupby(level="code")
                .apply(lambda s: s.droplevel(0).ffill())
                .droplevel(0)
            )
...
class OperatorLibrary:
    ...
    def apply_binary(
        ...
        if op == "bucket_transform":
            ...
            res = (
                ...
                .groupby(level="pred_date")
                .apply(lambda df: df.groupby("group")["lhs"].transform(mode if mode != "mean" else "mean"))
            )
            res = res.droplevel("pred_date")
    ...
    def _apply_bucket(self, series: pd.Series, buckets: int, mode: str) -> pd.Series:
        ...
        return series.groupby(level="pred_date", group_keys=False).apply(_transform)
...
class MetricBook:
    def __init__(self, target: pd.Series):
        if target.isna().all():
            raise ValueError("Target series cannot be entirely NaN.")
        self.target = target

    def evaluate(self, series: pd.Series) -> MetricResult:
        aligned = pd.concat([series, self.target], axis=1, join="inner").dropna()
        if aligned.empty:
            return MetricResult(ic=np.nan, lw_ic_v2=np.nan)
        grouped = aligned.groupby(level="pred_date")
        ic_list = []
        weights = []
        for _, block in grouped:
            if block.shape[0] < 5:
                continue
            rank_alpha = block.iloc[:, 0].rank()
            rank_target = block.iloc[:, 1].rank()
            corr = rank_alpha.corr(rank_target)
            if pd.isna(corr):
                continue
            ic_list.append(corr)
            weights.append(math.sqrt(block.shape[0]))
        if not ic_list:
            return MetricResult(ic=np.nan, lw_ic_v2=np.nan)
        ic_arr = np.asarray(ic_list)
        ic = float(ic_arr.mean())
        lw_ic = float(np.average(ic_arr, weights=weights))
        return MetricResult(ic=ic, lw_ic_v2=lw_ic)
...
class AlphaComboManager:
    ...
    def attempt_add(
        ...
        for variant_key, variant_series in variants.items():
            ...
            if metrics is None:
                metrics = self.metric_book.evaluate(variant_series)
            ...
            for weight in (2.0, 1.0, 0.5, 0.1):
                ...
                combined_metrics = self.metric_book.evaluate(combined_series)
...
def run_search(
    ...
    workspace = AlphaWorkspace(feature_df)
    operator_lib = OperatorLibrary(workspace)
    builder = AlphaBuilder(operator_lib)
    metric_book = MetricBook(target)
    combo_manager = AlphaComboManager(metric_book, log_path, combo_parquet)
...
def _parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--target-column",
        required=True,
        help="Name of the target column used for IC evaluation",
    )
    parser.add_argument(
        "--iterations",
        type=int,
